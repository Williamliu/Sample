...
LWH.CALENDAR = function(opts) {
	...
	var _constructor = function() {
			...
			$("a.lwhCalendar-removeall[scope='" + _self.table.head.scope + "']").die("click").live("click", function(ev) {
				_self.clear();
			});
			...
			$("input.lwhCalendar-picker[scope='" + _self.table.head.scope + "'][date]").die("click").live("click", function(ev) {
				if( parseInt(_self.table.head.multiple) ) {
					...
					$("input.lwhCalendar-picker[scope='" + _self.table.head.scope + "'][date]").attr("checked", false);
			...
			$(_self.syncObj.valObj).unbind("CalendarEvent").bind("CalendarEvent", function(ev, obj) {
				_self.valObj.val = (""+$(_self.syncObj.valObj).val()).split(",");
				_self.updateValue();
				_self.updateValueHTML();
			});

			$(_self.syncObj.timeHH).unbind("CalendarEvent").bind("CalendarEvent", function(ev, obj) {
				_self.valObj.timehh = $(_self.syncObj.timeHH).val();
				_self.updateValue();
				_self.updateValueHTML();
			});

			$(_self.syncObj.timeMM).unbind("CalendarEvent").bind("CalendarEvent", function(ev, obj) {
...
LWH.CALENDAR.prototype = {
	...
	toHTML: function(weeks) {
		...
		if( parseInt(this.table.head.width) > 0 ) 
			html += '<table class="lwhCalendar" style="width:' + parseInt(this.table.head.width) + 'px;">';
		else 
		...
		html += parseInt(this.table.head.btnRemove)?'<a scope="' +  this.table.head.scope + '" class="lwhCalendar-removeall" title="' + gcommon.trans[this.table.head.lang].words["remove all"] + '"></a>':'',
		
		/*
		html +=	['<a class="lwhCalendar-button-selection" style="margin-top:3px; margin-left:1px; margin-right:1px;">',
					'<div class="lwhCalendar-selectionarea">',
					'<div scope="' + this.table.head.scope + '" class="lwhCalendar-selectionarea-head">' + gcommon.trans[this.table.head.lang].words["selected"] + '</div>',
					'<ul scope="' + this.table.head.scope + '" class="lwhCalendar-selectionarea-content"></ul>',
					'</div>',
				'</a>'].join('');
		*/
		...
		for(var weekidx in weeks) {
			...
			for(var dayidx in weeks[weekidx]) {
				...
				var cellStyle = parseInt(this.table.head.height)>0?' style="height:'+ this.table.head.height +'px;"':''; 
				if( dateObj.date >= this.mindate && dateObj.date <= this.maxdate ) {
					...
					html += '</div>';
					html += '<input type="checkbox"  scope="' + this.table.head.scope + '" date="' + dateObj.ymd + '"  w="' + dateObj.W + '"  widx="' + dateObj.widx + '" ' + (this.valObj.val.indexOf(dateObj.ymd)>=0?'checked="checked"':'') + ' class="lwhCalendar-picker" value="' + dateObj.ymd + '" />';
					html += '</label>';
					html += '</td>';
				} else {
	...
	updateValueHTML: function() {
		...
		$("input.lwhCalendar-picker[scope='" + this.table.head.scope + "'][date]").attr("checked", false);

		for( var idx in this.valObj.val ) {
			$("td.date[scope='" + this.table.head.scope + "'][date='" + this.valObj.val[idx] + "']").addClass("date-selected");
			$("input.lwhCalendar-picker[scope='" + this.table.head.scope + "'][date='" + this.valObj.val[idx] + "']").attr("checked", true);
		}
